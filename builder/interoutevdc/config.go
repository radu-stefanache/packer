...
type Config struct {
	...
	VDCRegion    string        `mapstructure:"vdc_region"`
...
func NewConfig(raws ...interface{}) (*Config, error) {
	...
	// Set some defaults.
	c.APIURL = "https://myservices.interoute.com/myservices/api/vdc"

	if c.APIKey == "" {
		// Default to environment variable for api_key, if it exists
		c.APIKey = os.Getenv("INTEROUTE_VDC_API_KEY")
	}

	if c.SecretKey == "" {
		// Default to environment variable for secret_key, if it exists
		c.SecretKey = os.Getenv("INTEROUTE_VDC_SECRET_KEY")
	}
